...
name: ultra-query
description: >
  Generate queries and map visualizations for Ultra (Overpass Ultra), an advanced OpenStreetMap
  data query and visualization tool built on MapLibre. Use this skill whenever the user asks to
  create, edit, or debug an Ultra query, write an Overpass/Postpass/QLever query with map styling,
  visualize OpenStreetMap data on a map, or mentions "Ultra" in the context of mapping or OSM.
  Also trigger when the user asks for styled map queries involving OSM tags, even if they don't
  mention Ultra by name — phrases like "visualize sidewalks on a map", "show me bike lanes with
  styling", "query OSM and display results", or "make an Overpass query with custom icons" are
  strong signals. If the user pastes what looks like an Ultra query (YAML frontmatter + query body),
  this skill applies. Don't confuse with unrelated uses of "ultra" (e.g. "ultra marathon",
  "ultra settings").
---
